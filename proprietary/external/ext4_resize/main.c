...
int main(int argc, char* agrv[])
{
    ...
    if (STATS_RESIZED == stats.state) {
        ...
        if((ret = snprintf(size, 16, "%lluK", (unsigned long long)(stats.size / 1024))) < 0) {
        ...
        switch(pid) {
            case -1:
                ...
                break;
            case 0: // child 
                execl("/system/bin/e2fsck", "e2fsck", "-pDf", DATA_PATH, NULL);
                ALOGE("execl fail.\n");
                _exit(127);
                break;
            default: //parent 
...
static int get_resize_stats(struct resize_stats *stats)
{
    ...
    ALOGE("size for /dev/block/platform/mtk-msdc.0/by-name/data is %lluK.\n", data_partition_size / 1024);
